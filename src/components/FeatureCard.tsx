...
const FeatureCard: React.FC<FeatureCardProps> = ({ title, description, imageSrc, clientLogo, isEven }) => {
  return (
    <div className={`center flex-col sm:my-14  md:flex-row md:text-left sm:p-2 w-full  ${isEven ? "md:flex-row-reverse" : ""}`}>
      <div className="w-full md:w-1/2 center max-sm:mt-5">
  ...
  {title && <h3 className="para_head text-dark_gray ">
    <span className="text-primary">{title.split(" ")[0]}</span> {title.split(" ").slice(1).join(" ")}
        ...
        <p className="text-gray max-w-lg para">{description}</p>
      </div>
    </div>
  );
};
